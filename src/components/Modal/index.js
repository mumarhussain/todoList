...
function TodosModal({ modalOpen, setModalOpen }) {
  ...
  useEffect(() => {
    if (date && time) {
      ...
      const notificationKey = `${formattedDate}-${formattedTime}`;
      const hasNotificationBeenShown = localStorage.getItem(notificationKey);
      if (!hasNotificationBeenShown && secondsDiff > 0) {
      ...
      } else if (hasNotificationBeenShown) {
      }
    }
  }, [addToast, date, time]);
